...
* $Id: pdf.php.t 5067 2010-11-06 07:37:22Z naudefj $
*
...
function post_to_smiley($text)
{
	...
	if ($forum) {
		...
		if ($page) {
			if (!q_singleval('SELECT id FROM fud26_forum WHERE id='. $forum)) {
				invl_inp_err();
			}
			$lwi = q_singleval('SELECT seq FROM fud26_tv_'. $forum .' ORDER BY seq DESC LIMIT 1');
			if ($lwi === NULL || $lwi === FALSE) {
				invl_inp_err();
			}

			$join = 'FROM fud26_tv_'. $forum .' tv
	...
	if (!$sel) {
		...
		$c = q('SELECT p.*, u.alias, p.duser_id AS user_id FROM fud26_pmsg p 

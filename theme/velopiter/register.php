...
* $Id: register.php.t 5030 2010-10-08 18:27:42Z naudefj $
*
...
function generate_turing_val(&$rt)
{
	...
	if (($GLOBALS['FUD_OPT_3'] & 33554432) && extension_loaded('gd') && function_exists('imagecreate') ) {
		...
		return '<img src="index.php?t=captchaimg" alt="Антибот проверка. Вам нужно распознать текст на этом изображении." /><br />На изображении нет цифр 0 и 1.';
	} else {
...
function register_form_check($user_id)
{
	...
	if (!$user_id) {
		if (($reg_limit_reached = $GLOBALS['REG_TIME_LIMIT'] + q_singleval('SELECT join_date FROM fud26_users WHERE id='. q_singleval('SELECT MAX(id) FROM fud26_users')) - __request_timestamp__) > 0) {
			set_err('reg_time_limit', '<tr class="RowStyleA"><td class="ac ErrorText" colspan="2">Превышен предел на одну регистрацию в течение '.$GLOBALS['REG_TIME_LIMIT'].' секунд. Пожалуйста, подождите '.convertPlural($reg_limit_reached, array(''.$reg_limit_reached.' секунду',''.$reg_limit_reached.' секунды',''.$reg_limit_reached.' секунд')).' и попробуйте ещё раз.</td></tr>');
		}

		$_POST['reg_plaintext_passwd'] = trim($_POST['reg_plaintext_passwd']);

		if (strlen($_POST['reg_plaintext_passwd']) < 6) {
			set_err('reg_plaintext_passwd', 'Пароль должен состоять как минимум из 6 символов');
		}
		...
		if ($_POST['reg_plaintext_passwd'] !== $_POST['reg_plaintext_passwd_conf']) {
			set_err('reg_plaintext_passwd', 'Заданные пароли не совпадают.');
		}

		$_POST['reg_login'] = trim(sanitize_login($_POST['reg_login']));

		if (strlen($_POST['reg_login']) < 4) {
			set_err('reg_login', 'Выбрано слишком короткое имя учётной записи — оно должно быть не короче 4-х символов.');
		} else if (is_login_blocked($_POST['reg_login'])) {
			set_err('reg_login', 'Недопустимое имя участника.');
		} else if (get_id_by_login($_POST['reg_login'])) {
			set_err('reg_login', 'Имена участников форума должны быть уникальными, а заданное вами имя учётной записи уже занято другим участником.');
		}

		if (!($GLOBALS['FUD_OPT_3'] & 128)) { // Captcha not disabled.
			if (!test_turing_answer($_POST['turing_test'], $_POST['turing_res'])) {
				set_err('reg_turing', 'Неверный код подтверждения.');
			}
		}
		...
		if (validate_email($_POST['reg_email'])) {
			set_err('reg_email', 'Заданный вами адрес e-mail не похож на настоящий.');
		} else if (get_id_by_email($_POST['reg_email'])) {
			set_err('reg_email', 'Пользователь с таким адресом e-mail уже зарегистрирован. Если вы забыли пароль - используйте функцию его восстановления или сброса вместо повторной регистрации в форуме.');
		} else if (is_email_blocked($_POST['reg_email'])) {
			set_err('reg_email', 'Пользователь с таким адресом e-mail уже зарегистрирован. Если вы забыли пароль - используйте функцию его восстановления или сброса вместо повторной регистрации в форуме.');
		}
	} else {
		if (!($r = db_sab('SELECT id, passwd, salt, name, email FROM fud26_users WHERE id='. (!empty($_POST['mod_id']) ? __fud_real_user__ : $user_id)))) {
			exit('Go away!');
		}
		...
		if (empty($_POST['reg_confirm_passwd']) || !((empty($r->salt) && $r->passwd == md5($_POST['reg_confirm_passwd'])) || $r->passwd == sha1($r->salt . sha1($_POST['reg_confirm_passwd'])))) {
			if ($_POST['reg_email'] != $r->email || $_POST['reg_name'] != $r->name) {
				if (!empty($_POST['mod_id'])) {
					set_err('reg_confirm_passwd', 'Вы должны ввести пароль <b>администратора</b> для завершения изменений.');
				} else {
					set_err('reg_confirm_passwd', 'Вы должны ввести ваш текущий пароль для завершения изменений');
				}
			}
		}
		...
		if (validate_email($_POST['reg_email'])) {
			...
			set_err('reg_email', 'В системе уже зарегистрирован пользователь с таким адресом e-mail, и этот пользователь - не вы');
		}
	}
	...
	if (strlen($_POST['reg_name']) < 2) {
		set_err('reg_name', 'Для успешного завершения регистрации вы должны ввести свое имя.');
	}

	/* Image count check. */
	if ($GLOBALS['FORUM_IMG_CNT_SIG'] && $GLOBALS['FORUM_IMG_CNT_SIG'] < substr_count(strtolower($_POST['reg_sig']), '[img]') ) {
		set_err('reg_sig', 'Вы пытаетесь использовать большее количество картинок в подписи, чем максимально допустимое: '.$GLOBALS['FORUM_IMG_CNT_SIG'].'.');
	}

	/* URL Avatar check. */
	if (!empty($_POST['reg_avatar_loc']) && !($GLOBALS['reg_avatar_loc_file'] = fetch_img($_POST['reg_avatar_loc'], $user_id))) {
		set_err('avatar', 'Указанная вами ссылка URL не содержит картинки');
	}
	if (!empty($GLOBALS['reg_avatar_loc_file']) && filesize($GLOBALS['reg_avatar_loc_file']) >= $GLOBALS['CUSTOM_AVATAR_MAX_SIZE']) {
		set_err('avatar', 'Загруженный вами файл картинки слишком велик и превышает установленный предел в '.$GLOBALS['CUSTOM_AVATAR_MAX_SIZE'].' байт');
	}
	...
	if ($GLOBALS['FUD_OPT_2'] & 128 && isset($_POST['reg_alias'])) {
		if (($_POST['reg_alias'] = trim(sanitize_login($_POST['reg_alias'])))) {
			if (is_login_blocked($_POST['reg_alias'])) {
				set_err('reg_alias', 'Такой псевдоним недопустим');
			}
			if (q_singleval('SELECT id FROM fud26_users WHERE alias='. _esc(make_alias($_POST['reg_alias'])) .' AND id!='. $user_id)) {
				set_err('reg_alias', 'Псевдоним, который вы пытаетесь использовать, уже был занят другим пользователем. Смените его.');
			}
		}
	}

	if ($GLOBALS['FORUM_SIG_ML'] && strlen($_POST['reg_sig']) > $GLOBALS['FORUM_SIG_ML']) {
		set_err('reg_sig', 'Длина вашей подписи превышает допустимый максимум в '.$GLOBALS['FORUM_SIG_ML'].' characters символов.');
	}
	...
	if (preg_match('?(\[url)|(http://)|(https://)?i', $_POST['reg_sig'])) {
		...
		if ( $GLOBALS['POSTS_BEFORE_LINKS'] > $c ) {
			...
			set_err('reg_sig', 'Вы не можете использовать ссылки, пока вами отправлено менее '.convertPlural($posts_before_links, array(''.$posts_before_links.' сообщения',''.$posts_before_links.' сообщений',''.$posts_before_links.' сообщений')).'.');
		}
	}
...
/* main */
		...
		if (!($FUD_OPT_1 & 8) && (!@file_exists($avatar_arr['file']) || empty($avatar_arr['leave']))) {
			...
			if ($_FILES['avatar_upload']['size'] >= $CUSTOM_AVATAR_MAX_SIZE) {
				...
				if ($img_info[2] > ($FUD_OPT_1 & 64 ? 4 : 3)) {
					set_err('avatar', 'Загруженный вами файл картинки не принадлежит к одному из допустимых в системе типов файлов.');
					unlink($TMP . $tmp_name);
				} else if ($img_info[0] >$max_w || $img_info[1] >$max_y) {
					set_err('avatar', 'Размер картинки <b>('.$img_info[0].'x'.$img_info[1].')</b> превышает максимально разрешенный, который составляет <b>('.$GLOBALS['CUSTOM_AVATAR_MAX_DIM'].')</b>.');
					unlink($TMP . $tmp_name);
				} else {
	...
	if (isset($_POST['fud_submit']) && !isset($_POST['btn_detach']) && !isset($_POST['btn_upload']) && !register_form_check($uent->id)) {
		...
		if (!$uent->theme) {
			$uent->theme = q_singleval('SELECT id FROM fud26_themes WHERE theme_opt>=2 AND '. q_bitand('theme_opt', 2) .' > 0 LIMIT 1');
		}
		...
		if (!__fud_real_user__) { /* new user */
			...
			if ($FUD_OPT_2 & 1) {
				send_email($NOTIFY_FROM, $uent->email, 'Подтверждение регистрации', 'Спасибо за регистрацию,\nДля активации регистрации пожалуйста следуйте по приведенной ниже ссылке:\n\n'.$GLOBALS['WWW_ROOT'].'index.php?t=emailconf&conf_key='.$uent->conf_key.'\n\nПосле того, как регистрация активирована, вы автоматически войдете в форум\nи будете направлены на его главную страницу.', '');
			} else if (!($FUD_OPT_3 & 2048)) {
				send_email($NOTIFY_FROM, $uent->email, 'Подтверждение регистрации в форуме', 'Спасибо за регистрацию,\n\nНиже приведена информация о вашей регистрации:\n\nURL форума: index.php?t=index\nУчётная запись: '.$uent->login.'\nПароль: '.$_POST['reg_plaintext_passwd'].'\n\nИмейте ввиду, что пароли чувствительны к регистру символов!\nИзменить ваши персональные данные и настройки вы сможете по этому адресу:\n'.$GLOBALS['WWW_ROOT'].'index.php?t=register\n', '');
			}

			/* We notify all admins about the new user, so that they can approve him. */
			if (($FUD_OPT_2 & 132096) == 132096) {
				$admins = db_all('SELECT email FROM fud26_users WHERE users_opt>=1048576 AND '. q_bitand('users_opt', 1048576) .' > 0');
				send_email($NOTIFY_FROM, $admins, 'Зарегистрирован новый участник и ожидается утверждение его учетной записи', 'В форуме был зарегистрирован новый пользователь ('.$uent->login.'), и, поскольку включен режим утверждения пользователей, его учетная запись не будет активизирована до ее утверждения вами или другим администратором форума. Для утверждения учетной записи перейдите по ссылке: '.$GLOBALS['WWW_ROOT'].'adm/admaccapr.php\n\nЭто автоматически сгенерированное сообщение, не пытайтесь на него отвечать.\nЕсли вы хотите выключить извещения о регистрации новых пользователей в дальнейшем, сделайте это в панели управления, изменив значения параметра "Извещение о новых пользователях".', '');
			}
			...
			if ($FUD_OPT_2 & 1 && $old_email && $old_email != $uent->email && !($uent->users_opt & 1048576)) {
				...
				send_email($NOTIFY_FROM, $uent->email, 'Подтверждение смены адреса E-mail', 'Требуется подтверждение смены старого адреса e-mail "'.$old_email.'" на новый "'.$uent->email.'". Для подтверждения перейдите по приведенной ниже ссылке:\n'.$GLOBALS['WWW_ROOT'].'index.php?t=emailconf&conf_key='.$conf_key.'\n\nПосле подтверждения вами нового адреса e-mail учетная запись на форуме будет изменена.', '');
			}
			if (!$mod_id) {
				check_return($usr->returnto);
			} else {
			...
			error_dialog('ОШИБКА: регистрация невозможна', 'Невозможно создать учётную запись, пожалуйста сообщите об этом администратору по адресу <a href="mailto:'.$ADMIN_EMAIL.'">'.$ADMIN_EMAIL.'</a>');
		}
	}
	...
	if (!$mod_id) {
		if (__fud_real_user__) {
			ses_update_status($usr->sid, 'Просмотр собственных данных и настроек', 0, 0);
		} else {
			ses_update_status($usr->sid, 'Регистрационная страница', 0, 0);
		}
	}

	$TITLE_EXTRA = ': Регистрация';
...
// TODO: Setup a cache so that we don't have read from DB every time we need them.
	...
	$c = uq('SELECT id, name, descr, type_opt, choice, field_opt, vieworder FROM fud26_custom_fields ORDER BY vieworder');
	while ($r = db_rowobj($c)) {
...
/* main */
	...
	<td valign="top">'.$r->name.draw_err('custom_field_'. $r->id).'
	                 <br /><span class="SmallText">'.$r->descr.'</span></td>
	<td><textarea name="custom_field_'.$r->id.'" rows="5" cols="50">'.$val.'</textarea></td>
</tr>';
		} else if ($r->type_opt & 2) {	// # 2 == Select drop down.
			$custom_field_select = tmpl_draw_select_opt($r->choice, $r->choice, $custom_field_vals[$r->id]);
			$custom_field = '<tr class="RowStyleA">
	<td>'.$r->name.draw_err('custom_field_'. $r->id).'
	    <br /><span class="SmallText">'.$r->descr.'</span></td>
	<td><select name="custom_field_'.$r->id.'">'.$custom_field_select.'</select></td>
</tr>';
		} else if ($r->type_opt & 4) {	// # 4 == Radio buttons.
			$custom_field_radio = tmpl_draw_radio_opt('custom_field_'. $r->id, $r->choice, $r->choice, $custom_field_vals[$r->id], '&nbsp;&nbsp;');
			$custom_field = '<tr class="RowStyleA">
		<td>'.$r->name.draw_err('custom_field_'. $r->id).'
		    <br /><span class="SmallText">'.$r->descr.'</span></td>
		<td>'.$custom_field_radio.'</td>
</tr>';
		} else {	// # 0 == Single line.
			$val = empty($custom_field_vals[$r->id]) ? $r->choice : $custom_field_vals[$r->id];
			$custom_field = '<tr class="RowStyleA">
	<td>'.$r->name.draw_err('custom_field_'. $r->id).'
	    <br /><span class="SmallText">'.$r->descr.'</span></td>
	<td><input type="text" name="custom_field_'.$r->id.'" value="'.$val.'" maxlength="255" size="30" /></td>
</tr>';
		}
	...
	if (__fud_real_user__) {
		if ($uent->users_opt & 131072 && $FUD_OPT_2 & 1 && !($uent->users_opt & 1048576)) {
			$email_warning_msg = '<br /><span class="regEW">В случае изменения вами вашего адреса e-mail, регистрация будет помечена как неподтвержденная до получения подтверждения с нового e-mail адреса.</span>';
		} else {
			$email_warning_msg = '';
		}
		...
		if ($FUD_OPT_1 & 28 && _uid) {
			...
			if ($a_type) { /* Rare condition, no built-in avatars & no other avatars are allowed. */
				...
				$avatar_type_sel = '<tr class="vt RowStyleA"><td>Тип персональной картинки:</td><td><select name="avatar_type" onchange="document.forms[\'fud_register\'].submit();">'.$avatar_type_sel_options.'</select></td></tr>';
				...
				if ($avatar_type == 'b') {
					...
					$del_built_in_avatar = $reg_avatar ? '[<a href="javascript://" onclick="document.reg_avatar_img.src=\'blank.gif\'; document.forms[\'fud_register\'].reg_avatar.value=\'0\';">Удалить картинку</a>]' : '';
					$avatar = '<tr class="vt RowStyleA"><td>Персональная картинка:</td><td><img src="'.$reg_avatar_img.'" name="reg_avatar_img" alt="" />
<input type="hidden" name="reg_avatar" value="'.$reg_avatar.'" />[<a href="javascript: window_open(\''.$GLOBALS['WWW_ROOT'].'index.php?t=avatarsel&amp;'._rsid.'\', \'avtsel\', 400, 300);">Выбрать картинку</a>]
'.$del_built_in_avatar.'<br /></td></tr>';
				} else if ($avatar_type == 'c') {
					if (!isset($reg_avatar_loc)) {
						$reg_avatar_loc = '';
					}
					$avatar = '<tr class="RowStyleC vt"><td colspan="2">Внешняя картинка не появится в ваших данных, пока не будет утверждена администратором форума.<br /><font class="SmallText">Размер картинки не должен превышать <b>'.$GLOBALS['CUSTOM_AVATAR_MAX_DIM'].' точек</b> и должна быть в формате <b>jpg</b>, <b>gif</b> или <b>png</b>.</font></td></tr>
<tr class="vt RowStyleA"><td>URL картинки: '.draw_err('avatar').'</td><td><input type="text" value="'.$reg_avatar_loc.'" name="reg_avatar_loc" /></td></tr>';
				} else if ($avatar_type == 'u') {
					$avatar_tmp = $avatar_arr ? base64_encode($avatar_arr['file'] . "\n" . $avatar_arr['del'] . "\n" . $avatar_arr['leave']) : '';
					$buttons = (!empty($avatar_arr['file']) && empty($avatar_arr['del'])) ? '&nbsp;<input type="submit" class="button" name="btn_detach" value="Удалить картинку" />' : '<input type="file" name="avatar_upload" /> <input type="submit" class="button" name="btn_upload" value="Предварительный просмотр" />
<input type="hidden" name="tmp_f_val" value="1" />';
					$avatar = '<tr class="RowStyleC vt"><td colspan="2">Внешняя картинка не появится в ваших данных, пока не будет утверждена администратором форума.<br /><font class="SmallText">Размер картинки не должен превышать <b>'.$GLOBALS['CUSTOM_AVATAR_MAX_DIM'].' точек</b> и должна быть в формате <b>jpg</b>, <b>gif</b> или <b>png</b>.</font></td></tr>
<tr class="vt RowStyleA"><td>Файл с картинкой: '.draw_err('avatar').'</td><td>
	<table border="0" cellspacing="0" cellpadding="0"><tr><td>'.$custom_avatar_preview.'</td><td>'.$buttons.'<input type="hidden" name="avatar_tmp" value="'.$avatar_tmp.'" /></td></tr></table>
</td></tr>';
				}
			}
		}
	...
	$theme_select = '';
	$r = uq('SELECT id, name FROM fud26_themes WHERE theme_opt>=1 AND '. q_bitand('theme_opt', 1) .' > 0 ORDER BY '. q_bitand('theme_opt', 2) .' DESC, name');
	...
	$views[384] = 'Плоский вид';
	if (!($FUD_OPT_3 & 2)) {
		$views[128] = 'Плоские форумы/Древовидные темы';
	}
	if ($FUD_OPT_2 & 512) {
		$views[256] = 'Древовидные форумы/Плоские темы';
		if (!($FUD_OPT_3 & 2)) {
			$views[0] = 'Древовидный';
		}
	}
	...
	$gender_select		= tmpl_draw_select_opt("512\n1024\n0","НЕ УКАЗАНО\nМужской\nЖенский", ($uent->users_opt & 512 ? 512 : ($uent->users_opt & 1024)));
	$mppg_select		= tmpl_draw_select_opt("0\n5\n10\n20\n30\n40", "Использовать умолчания форума\n5\n10\n20\n30\n40", $reg_posts_ppg);
...
<tr><th colspan="2">Обязательная информация</th></tr>
<tr><td colspan="2" class="RowStyleC">Все поля являются обязательными. Пожалуйста имейте ввиду, что пароли чувствительны к регистру символов.</td></tr>
<?php echo (!__fud_real_user__ ? (!__fud_real_user__ ? draw_err('reg_time_limit').'' : '' )  .'
<tr class="RowStyleA"><td width="60%">Учётная запись:'.draw_err('reg_login').'</td><td width="60%"><input type="text" size="25" name="reg_login" value="'.$reg_login.'" maxlength="'.$GLOBALS['MAX_LOGIN_SHOW'].'" /></td></tr>
'.($FUD_OPT_2 & 128 ? '<tr class="RowStyleA"><td>Псевдоним'.draw_err('reg_alias').'<br /><span class="SmallText">Если вы хотите использовать в форуме имя, отличное от того, с которым вы входите, укажите здесь псевдоним, который будет использоваться вместо имени.</span></td><td><input type="text" name="reg_alias" size="25" value="'.$reg_alias.'" maxlength="'.$GLOBALS['MAX_LOGIN_SHOW'].'" /></td></tr>' : '' )  .'
<tr class="RowStyleA"><td>Пароль:'.draw_err('reg_plaintext_passwd').'</td><td><input type="password" name="reg_plaintext_passwd" id="reg_plaintext_passwd" size="25" /></td></tr>
<tr class="RowStyleA"><td>Пароль повторно (для проверки):</td><td><input type="password" name="reg_plaintext_passwd_conf" id="reg_plaintext_passwd_conf" size="25" onkeyup="passwords_match(\'reg_plaintext_passwd\', this); return false;" /></td></tr>
<tr class="RowStyleA"><td>E-mail адрес:'.draw_err('reg_email').'<br /><span class="SmallText">Введите ваш реальный адрес e-mail. Скрыть его вы сможете в персональных настройках ниже.</span></td><td><input type="text" name="reg_email" size="25" value="'.$reg_email.'" /></td></tr>
<tr class="RowStyleA"><td>Имя:'.draw_err('reg_name').'</td><td><input type="text" name="reg_name" size="25" value="'.$reg_name.'" /></td></tr>
'.(!($FUD_OPT_3 & 128) ? '<tr class="RowStyleA"><td>Пожалуйста, введите код подтверждения, который вы видите ниже:'.draw_err('reg_turing').'<br /><div style="white-space: pre; font-family: Courier, monospace; color: black; background-color: #C0C0C0;">'.generate_turing_val($turing_res).'<input type="hidden" name="turing_res" value="'.$turing_res.'" /></div></td><td class="vb"><input type="text" name="turing_test" value="" /></td></tr>' : '' )  : '<tr class="RowStyleA"><td width="60%">Учётная запись:</td><td><span class="fb">'.$reg_login.'</span>'.(($FUD_OPT_4 & 1) && !$mod_id ? '&nbsp; <span class="SmallText">[ <a href="javascript://" onclick="window_open(\''.$GLOBALS['WWW_ROOT'].'index.php?t=ruser&amp;'._rsid.'\',\'ruser\',470,250);">изменить логин</a> ]</span>' : '' )  .'</td></tr>
'.($FUD_OPT_2 & 128 ? '<tr class="RowStyleA"><td>Псевдоним'.draw_err('reg_alias').'<br /><span class="SmallText">Если вы хотите использовать в форуме имя, отличное от того, с которым вы входите, укажите здесь псевдоним, который будет использоваться вместо имени.</span></td><td><input type="text" name="reg_alias" size="25" value="'.$reg_alias.'" maxlength="'.$GLOBALS['MAX_LOGIN_SHOW'].'" /></td></tr>' : '' )  .'
<tr class="RowStyleA"><td>Ваш пароль:'.draw_err('reg_confirm_passwd').'</td><td><nobr><input type="password" name="reg_confirm_passwd" size="25" />'.(($FUD_OPT_4 & 2) && !$mod_id ? '&nbsp; <span class="SmallText">[ <a href="javascript://" onclick="window_open(\''.$GLOBALS['WWW_ROOT'].'index.php?t=rpasswd&amp;'._rsid.'\',\'rpass\',470,250);">изменить пароль</a> ]</span>' : '' )  .'</nobr></td></tr>
<tr class="RowStyleA"><td>E-mail адрес:'.draw_err('reg_email').'<br /><span class="SmallText">Введите ваш реальный адрес e-mail. Скрыть его вы сможете в персональных настройках ниже.</span>'.$email_warning_msg.'</td><td><input type="text" name="reg_email" size="25" value="'.$reg_email.'" /></td></tr>
<tr class="RowStyleA"><td>Имя:'.draw_err('reg_name').'</td><td><input type="text" name="reg_name" size="25" value="'.$reg_name.'" /></td></tr>' )  .'
'.$required_custom_fields.'
'.(__fud_real_user__ ? '<tr><td colspan="2" class="ac RowStyleC"><input type="submit" class="button" name="fud_submit" value="Обновить" /></td></tr>' : ''); ?>
<tr><th colspan="2">Дополнительная информация</th></tr>
<tr><td colspan="2" class="RowStyleC">Не рекомендуется публиковать какие-либо чересчур личные данные, поскольку вся указанная ниже информация доступна всем пользователям форума.</td></tr>
<?php echo $optional_custom_fields; ?>
<tr class="RowStyleA"><td>Географическое положение:</td><td><input type="text" spellcheck="true" name="reg_location" value="<?php echo $reg_location; ?>" maxlength="255" size="30" /></td></tr>
<tr class="RowStyleA"><td>Род деятельности:</td><td><input type="text" spellcheck="true" name="reg_occupation" value="<?php echo $reg_occupation; ?>" maxlength="255" size="30" /></td></tr>
<tr class="RowStyleA"><td>Интересы:</td><td><input type="text" spellcheck="true" name="reg_interests" value="<?php echo $reg_interests; ?>" maxlength="255" size="30" /></td></tr>
<?php echo $avatar_type_sel; ?>
<?php echo $avatar; ?>
<tr class="RowStyleA vt"><td>Дата рождения:<br /><span class="SmallText">При указании вами даты своего рождения, остальные пользователи<br />смогут ее увидеть при просмотре вашего профиля.</span></td>
<td>
<table border="0" cellspacing="3" cellpadding="0">
 <tr class="GenText">
  <td class="ac">Месяц</td>
  <td class="ac">День</td>
  <td class="ac">Год</td>
 </tr>
 <tr>
...
<tr class="RowStyleA"><td>Пол:</td><td><select name="reg_gender"><?php echo $gender_select; ?></select></td></tr>
<?php echo ($FUD_OPT_2 & 65536 ? '<tr class="RowStyleA"><td>Картинка:</td><td><input type="text" name="reg_user_image" value="'.$reg_user_image.'" maxlength="255" size="30" /></td></tr>' : ''); ?>
<tr class="RowStyleA"><td>Домашняя страница или сайт:</td><td><input type="text" name="reg_home_page" value="<?php echo $reg_home_page; ?>" maxlength="255" /></td></tr>
<tr class="RowStyleA"><td class="RowStyleA" valign="top">Биография:<br /><span class="SmallText">Отдельные детали, такие как ваши интересы, работа и т. п...</span></td><td><textarea name="reg_bio" rows="5" cols="50"><?php echo $reg_bio; ?></textarea></td></tr>
<tr class="RowStyleA"><td colspan="2">
<fieldset class="RowStyleA"><legend class="RowStyleB">Сайты социальных сетей:</legend>
<table border="0" cellspacing="3" cellpadding="5" align="center">
<tr><td>
	<label>ICQ<br /><img src="theme/velopiter/images/icq<?php echo img_ext; ?>" alt="" /><input type="text" name="reg_icq" value="<?php echo $reg_icq; ?>" maxlength="32" size="25" /></label>
</td><td>
	<label>Имя в сети AIM:<br /><img src="theme/velopiter/images/aim<?php echo img_ext; ?>" alt="" /><input type="text" name="reg_aim" value="<?php echo $reg_aim; ?>" maxlength="32" size="25" /></label>
</td></tr><tr><td>
	<label>Имя в сети Yahoo:<br /><img src="theme/velopiter/images/yahoo<?php echo img_ext; ?>" alt="" /><input type="text" name="reg_yahoo" value="<?php echo $reg_yahoo; ?>" maxlength="32" size="25" /></label>
</td><td>
	<label>Имя в сети MSN:<br /><img src="theme/velopiter/images/msnm<?php echo img_ext; ?>" alt="" /><input type="text" name="reg_msnm" value="<?php echo $reg_msnm; ?>" maxlength="32" size="25" /></label>
</td></tr><tr><td>
	<label>Имя в сети Jabber:<br /><img src="theme/velopiter/images/jabber<?php echo img_ext; ?>" alt="" /><input type="text" name="reg_jabber" value="<?php echo $reg_jabber; ?>" maxlength="32" size="25" /></label>
</td><td>
	<label>Имя в сети Google Chat/IM:<br /><img src="theme/velopiter/images/google<?php echo img_ext; ?>" alt="" /><input type="text" name="reg_google" value="<?php echo $reg_google; ?>" maxlength="32" size="25" /></label>
</td></tr><tr><td>
	<label>Имя в Skype:<br /><img src="theme/velopiter/images/skype<?php echo img_ext; ?>" alt="" /><input type="text" name="reg_skype" value="<?php echo $reg_skype; ?>" maxlength="32" size="25" /></label>
</td><td>
	<label>Прозвище в Twitter:<br /><img src="theme/velopiter/images/twitter<?php echo img_ext; ?>" alt="" /><input type="text" name="reg_twitter" value="<?php echo $reg_twitter; ?>" maxlength="32" size="25" /></label>
<?php echo ($FUD_OPT_2 & 2048 ? '
</td></tr><tr><td>
	<label>Имя в системе Affero:<br /><font class="SmallText">Если у вас имеется зарегистрированное имя в системе <a href="http://www.affero.com/ca/'.urlencode($affero_domain['host']).'" target="_blank">Affero</a> — введите его здесь.</font><br /><input type="text" name="reg_affero" value="'.$reg_affero.'" maxlength="32" size="25" /></label>
' : '' )  .'
</td></tr></table>
</fieldset>
<br />
</td></tr>
<tr><th colspan="2">Персональные настройки</th></tr>
'.($FUD_OPT_1 & 32768 ? '
<tr class="RowStyleA"><td class="vt">Подпись:<br /><span class="SmallText">Необязательная подпись, которая будет включаться в ваши сообщения.<br />'.tmpl_post_options('sig').($FORUM_SIG_ML ? '<br /><b>Максимальная длинна: </b>'.$GLOBALS['FORUM_SIG_ML'].' characters <a href="javascript: alert(&#39;Длина вашей подписи составляет &#39;+document.forms[&#39;fud_register&#39;].reg_sig.value.length+&#39; символов. Максимально допустимая длина - '.$GLOBALS['FORUM_SIG_ML'].'.&#39;);" class="SmallText">Проверить длину подписи</a>' : '' )  .'</span></td><td>'.draw_err('reg_sig').'<textarea name="reg_sig" rows="5" cols="50">'.$reg_sig.'</textarea></td></tr>
' : ''); ?>
<tr class="RowStyleA"><td>Часовой пояс:</td><td><select name="reg_time_zone"><?php echo $timezone_select; ?></select></td></tr>
<tr class="RowStyleA"><td>Игнорирование административных сообщений:</td><td><?php echo $ignore_admin_radio; ?></td></tr>
<tr class="RowStyleA"><td>Режим невидимости:<br /><span class="SmallText">Скрывает ваши действия от других пользователей.</span></td><td><?php echo $invisible_mode_radio; ?></td></tr>
<tr class="RowStyleA"><td>Показывать адрес e-mail:<br /><span class="SmallText">Включите, если хотите чтобы ваш адрес e-mail был видим другими пользователями.</span></td><td><?php echo $show_email_radio; ?></td></tr>
<tr class="RowStyleA"><td>Включить извещения по умолчанию:<br /><span class="SmallText">При включенных по умолчанию извещениях из можно отключить при отправке сообщений.</span></td><td><?php echo $notify_default_radio; ?></td></tr>
<tr class="RowStyleA"><td>Уведомления о личных сообщениях<br /><span class="SmallText">Получать уведомления о новых личных сообщениях способом, указанным ниже.</span></td><td><?php echo $pm_notify_default_radio; ?></td></tr>
<tr class="RowStyleA"><td>Метод отправки извещений:<br /><span class="SmallText">Укажите способ отправки вам извещений или отключите их (например если вы уезжаете в отпуск)</span></td><td><select name="reg_notify_method"><?php echo $notification_select; ?></select></td></tr>
<tr class="RowStyleA"><td>Разрешить сообщения на адрес E-mail:<br /><span class="SmallText">Разрешает другим участникам посылать через форум сообщения на ваш адрес электронной почты.</span></td><td><?php echo $accept_user_email; ?></td></tr>
<tr class="RowStyleA"><td>Разрешить личные сообщения<br /><span class="SmallText">Разрешить обмен личными сообщениями с другими пользователями форума.</span></td><td><?php echo $accept_pm; ?></td></tr>
<tr class="RowStyleA"><td>Использовать подпись по умолчанию:<br /><span class="SmallText">Автоматически добавлять подпись к любому отправленному вами сообщению</span></td><td><?php echo $append_sig_radio; ?></td></tr>
<tr class="RowStyleA"><td>Показывать подписи:<br /><span class="SmallText">Позволяет вам включить или отключить показ подписей других участников</span></td><td><?php echo $show_sig_radio; ?></td></tr>
<tr class="RowStyleA"><td>Показывать картинки:<br /><span class="SmallText">Показывать картинки других пользователей при просмотре их сообщений</span></td><td><?php echo $show_avatar_radio; ?></td></tr>
<tr class="RowStyleA"><td>Показывать сетевые онлайн-индикаторы<br /><span class="SmallText">Показывать в сообщениях индикатор онлайн-статуса автора в сетях ICQ, AIM и т.п.</span></td><td><?php echo $show_im_radio; ?></td></tr>
<tr class="RowStyleA"><td>Сообщений на странице:</td><td><select name="reg_posts_ppg"><?php echo $mppg_select; ?></select></td></tr>
<tr class="RowStyleA"><td>Количество тем на странице:</td><td><select name="reg_topics_per_page"><?php echo $topics_per_page; ?></select></td></tr>
<tr class="RowStyleA"><td>Вид тем по умолчанию:</td><td><select name="reg_default_view"><?php echo $view_select; ?></select></td></tr>
<?php echo ($theme_select ? '<tr class="RowStyleA"><td>Тема:</td><td><select name="reg_theme">'.$theme_select.'</select></td></tr>' : ''); ?>
<tr class="RowStyleC"><td colspan="2" class="ac"><?php echo (!__fud_real_user__ ? '<input type="submit" class="button" name="fud_submit" value="Регистрация" />' : '<input type="submit" class="button" name="fud_submit" value="Обновить" />'); ?>&nbsp;<input type="reset" class="button" name="Reset" value="Сброс" /></td></tr>
</table>
<?php echo _hs; ?>

...
}function alt_var($key)
{
	...
	if (!empty($_POST['f_unsub_all'])) {
		q('DELETE FROM fud26_forum_notify WHERE user_id='. _uid);
	} else if (!empty($_POST['t_unsub_all'])) {
		q('DELETE FROM fud26_thread_notify WHERE user_id='. _uid);
	} else if (isset($_POST['f_unsub_sel'], $_POST['fe'])) {
		$list = array();
		foreach((array)$_POST['fe'] as $v) {
			$list[(int)$v] = (int) $v;
		}
		q('DELETE FROM fud26_forum_notify WHERE user_id='. _uid .' AND forum_id IN('. implode(',', $list) .')');
	} else if (isset($_POST['t_unsub_sel'], $_POST['te'])) {
		$list = array();
		foreach((array)$_POST['te'] as $v) {
			$list[(int)$v] = (int) $v;
		}
		q('DELETE FROM fud26_thread_notify WHERE user_id='. _uid .' AND thread_id IN('. implode(',', $list) .')');
	}

	ses_update_status($usr->sid, 'Просмотр личной подписки');
...
<tr>'.$tabs.'</tr>
	...
	while (($r = db_rowarr($c))) {
		$subscribed_forum_data .= '<tr class="'.alt_var('search_alt','RowStyleA','RowStyleB').'"><td><input type="checkbox" name="fe[]" value="'.$r[0].'" /></td><td class="nw"><a href="index.php?t=subscribed&amp;frm_id='.$r[0].'&amp;'._rsid.'&amp;SQ='.$GLOBALS['sq'].'">Отписаться от форума</a><td class="wa"><a href="index.php?t='.t_thread_view.'&amp;frm_id='.$r[0].'&amp;'._rsid.'">'.$r[1].'</a></td></td></tr>';
	}
	unset($c);
	...
	$c = uq(q_limit('SELECT /*!40000 SQL_CALC_FOUND_ROWS */ t.id, m.subject, f.name FROM fud26_thread_notify tn INNER JOIN fud26_thread t ON tn.thread_id=t.id INNER JOIN fud26_forum f ON f.id=t.forum_id INNER JOIN fud26_msg m ON t.root_msg_id=m.id WHERE tn.user_id='. _uid .' '. $lmt .' ORDER BY t.last_post_id DESC',
			$THREADS_PER_PAGE, $start));

	while (($r = db_rowarr($c))) {
		$subscribed_thread_data .= '<tr class="'.alt_var('search_alt','RowStyleA','RowStyleB').'"><td><input type="checkbox" name="te[]" value="'.$r[0].'" /></td><td class="nw"><a href="index.php?t=subscribed&amp;th='.$r[0].'&amp;'._rsid.'&amp;SQ='.$GLOBALS['sq'].'">Отписаться от форума</a></td><td class="wa">'.$r[2].' &raquo; <a href="index.php?t='.d_thread_view.'&amp;th='.$r[0].'&amp;unread=1&amp;'._rsid.'">'.$r[1].'</a></td></tr>';
	}
	unset($c);
...
<tr><th colspan="3">Подписанные форумы<a name="fff"></a></th></tr>
<?php echo ($subscribed_forum_data ? '
'.$subscribed_forum_data.'
<tr class="RowStyleC"><td class="ac" colspan="2"><input type="submit" class="button" name="f_unsub_sel" value="Отписаться от форума" /></td><td class="ar"><input type="submit" class="button" name="f_unsub_all" value="Отписаться от всех форумов" /></td></tr>
' : '
<tr class="'.alt_var('search_alt','RowStyleA','RowStyleB').'"><td colspan="3">Нет форумов в подписке</td></tr>
'); ?>
<tr><th colspan="3">Подписанные темы</th></tr>
<?php echo ($subscribed_thread_data ? '
'.$subscribed_thread_data.'
<tr class="RowStyleC"><td class="ac" colspan="2"><input type="submit" class="button" name="t_unsub_sel" value="Отписаться от форума" /></td><td class="ar"><input type="submit" class="button" name="t_unsub_all" value="Отписаться от всех тем" /></td></tr>
' : '
<tr class="'.alt_var('search_alt','RowStyleA','RowStyleB').'"><td colspan="3">Нет тем в подписке</td></tr>
'); ?>
</table>
</form>
<?php echo $pager; ?>

...
function tmpl_create_pager($start, $count, $total, $arg, $suf='', $append=1, $js_pager=0, $no_append=0)
{
	...
	ses_update_status($usr->sid, '<a href="index.php?t=polllist">Просмотр доступных голосований</a>');
...
if (_uid) {
	...
	while ($obj = db_rowobj($c)) {
		$view_res_lnk = $obj->total_votes ? '<b>::</b> <a href="index.php?t=polllist&amp;goto='.$obj->id.'&amp;vote=1&amp;'._rsid.'#msg_'.$obj->id.'">Результаты</a>' : '';
		if (!$obj->total_votes) {
			$obj->total_votes = '0';
		}
		if ($obj->owner && (!($obj->users_opt & 32768) || $is_a) && $FUD_OPT_2 & 32) {
			$online_indicator = $obj->last_visit > __request_timestamp__ ? '<img src="theme/velopiter/images/online'.img_ext.'" title="онлайн" alt="онлайн" />&nbsp;' : '<img src="theme/velopiter/images/offline'.img_ext.'" title="оффлайн" alt="оффлайн" />&nbsp;';
		} else {
			$online_indicator = '';
		}
	...
	<td class="nw ac"> <b>::</b> <a href="index.php?t=polllist&amp;goto='.$obj->id.'&amp;'._rsid.'#msg_'.$obj->id.'">Голосовать</a> '.$view_res_lnk.' <b>::</b></td>
</tr>';
	}
	unset($c);
	...
	if ($ttl > $POLLS_PER_PAGE) {
		...
		$poll_entries = '<tr><td colspan="5" class="ac">Нет доступных голосований.</td></tr>';
	}
...
<tr>
	...
	<th class="nw"><a class="thLnk" href="index.php?t=polllist&amp;start=<?php echo $start; ?>&amp;oby=<?php echo $oby_rev_val; ?>&amp;<?php echo _rsid; ?>">Создано</a></th>
	<th class="nw">Создатель</th>
	<th class="nw ac">Итого голосов</th>
	<th class="nw ac">Действие</th>
</tr>

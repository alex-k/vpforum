...
}function read_msg_body($off, $len, $id)
{
	...
	if (empty($_GET['conf_key'])) {
		error_dialog('Ошибка', 'Недопустимый ключ подтверждения. Пожалуйста проверьте еще раз вашу электронную почту и убедитесь что ввели URL правильно.');
	}
	...
	$uid = q_singleval('SELECT id FROM fud26_users WHERE conf_key='. _esc($_GET['conf_key']));
	if (!$uid || (__fud_real_user__ && __fud_real_user__ != $uid)) {
		error_dialog('Ошибка', 'Недопустимый ключ подтверждения. Пожалуйста проверьте еще раз вашу электронную почту и убедитесь что ввели URL правильно.');
	}
	q('UPDATE fud26_users SET users_opt='. q_bitor(users_opt, 131072) .', conf_key=NULL WHERE id='. $uid);

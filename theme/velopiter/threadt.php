							...
							if ($TREE_THREADS_MAX_DEPTH > $lev) {
								...
								if (_uid) {
									if ($usr->last_read < $cur->post_stamp && $cur->post_stamp>$cur->last_view) {
										$thread_read_status = $cur->thread_opt & 1 ? '<img src="theme/velopiter/images/unreadlocked'.img_ext.'" title="Закрытая тема с непрочитанными сообщениями" alt="" />'	: '<img src="theme/velopiter/images/unread'.img_ext.'" title="Эта тема содержит не прочитанные вами сообщения" alt="" />';
									} else {
										$thread_read_status = $cur->thread_opt & 1 ? '<img src="theme/velopiter/images/readlocked'.img_ext.'" title="Эта тема была закрыта" alt="" />' : '<img src="theme/velopiter/images/read'.img_ext.'" title="В этой теме нет непрочитанных сообщений" alt="" />';
									}
								} else {
									$thread_read_status = $cur->thread_opt & 1 ? '<img src="theme/velopiter/images/readlocked'.img_ext.'" title="Эта тема была закрыта" alt="" />' : '<img src="theme/velopiter/images/read'.img_ext.'" title="Отслеживание прочитанных и непрочитанных сообщений доступно только зарегистрированным участникам" alt="" />';
								}
...
<td title="'.$cur->tdescr.'" class="tt" style="padding-left: '.(20 * ($lev - 1)).'px">'.($cur->poll_id ? 'Голосование:&nbsp;' : '' ) .($cur->attach_cnt ? '<img src="theme/velopiter/images/attachment.gif" alt="" />' : '' ) .'<a href="index.php?t='.d_thread_view.'&amp;goto='.$cur->id.'&amp;'._rsid.'#msg_'.$cur->id.'" class="big">'.$cur->subject.'</a>
...
<td class="GenText nw vb ar"><a href="index.php?t=thread&amp;frm_id=<?php echo $frm->id; ?>&amp;<?php echo _rsid; ?>"><img alt="Вернуться к плоскому виду по умолчанию" title="Вернуться к плоскому виду по умолчанию" src="theme/velopiter/images/flat_view.gif" /></a>&nbsp;<a href="index.php?t=post&amp;frm_id=<?php echo $frm->id; ?>&amp;<?php echo _rsid; ?>"><img src="theme/velopiter/images/new_thread.gif" alt="Создать новую тему" /></a></td>
</tr></table>

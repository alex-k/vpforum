...
}function alt_var($key)
{
	...
	if (isset($_POST['add_login']) && is_string($_POST['add_login'])) {
		if (!($buddy_id = q_singleval('SELECT id FROM fud26_users WHERE alias='. _esc(char_fix(htmlspecialchars($_POST['add_login'])))))) {
			error_dialog('Невозможно добавить пользователя', 'Пользователь, которого вы пытаетесь добавить в список контактов, не найден.');
		}
		if ($buddy_id == _uid) {
		...
		if (q_singleval('SELECT id FROM fud26_user_ignore WHERE user_id='. $buddy_id .' AND ignore_id='. _uid)) {
			error_dialog('Информация', 'Не могу добавить пользователей, которые Вас игнорируют.');
		}
...
<tr>'.$tabs.'</tr>
	...
	if (($r = db_rowarr($c))) {
		...
		do {
			if ((!($r[5] & 32768) && $FUD_OPT_2 & 32) || $is_a) {
				$online_status = (($r[8] + $LOGEDIN_TIMEOUT * 60) > __request_timestamp__) ? '<img src="theme/velopiter__old26/images/online'.img_ext.'" title="'.$r[2].' сейчас в онлайне" alt="'.$r[2].' сейчас в онлайне" />' : '<img src="theme/velopiter__old26/images/offline'.img_ext.'" title="'.$r[2].' сейчас не в онлайне" alt="'.$r[2].' сейчас не в онлайне" />';
			} else {
				$online_status = '';
			}
			...
			if ($r[4] && substr($r[4], 0, 4) == $md) {
				...
				$bday_indicator = '<img src="blank.gif" alt="" width=10 height=1 /><img src="theme/velopiter__old26/images/bday.gif" alt="" />Сегодня '.$r[2].' исполняется '.$age;
			} else {
				$bday_indicator = '';
			}
	...
	<td class="GenText wa">'.($FUD_OPT_1 & 1024 ? '<a href="index.php?t=ppost&amp;'._rsid.'&amp;toi='.urlencode($r[0]).'">'.$r[2].'</a>' : '<a href="index.php?t=email&amp;toi='.$r[1].'&amp;'._rsid.'">'.$r[2].'</a>' ) .'&nbsp;<span class="SmallText">(<a href="index.php?t=buddy_list&amp;'._rsid.'&amp;del='.$r[0].'&amp;SQ='.$GLOBALS['sq'].'">удалить</a>)</span>&nbsp;'.$bday_indicator.'</td>
	<td class="ac">'.$r[6].'</td>
...
if ($FUD_OPT_2 & 2 || $is_a) {	// PUBLIC_STATS is enabled or Admin user.
	...
	$page_stats = $FUD_OPT_2 & 2 ? '<br /><div class="SmallText al">Время, затраченное на генерацию страницы: '.$page_gen_time.' сек.</div>' : '<br /><div class="SmallText al">Время, затраченное на генерацию страницы: '.$page_gen_time.' сек.</div>';
} else {
	$page_stats = '';
}
?>
...
<tr><th nowrap>Добавить в список контактов</th></tr>
<tr class="RowStyleA">
<td class="GenText nw Smalltext">Введите имя пользователя, которого вы хотите добавить.<?php echo (($FUD_OPT_1 & 8388608 || (_uid && $FUD_OPT_1 & 4194304)) ? '<br>Или используйте возможность <a href="javascript://" onClick="javascript: window_open(\&#39;'.$GLOBALS['WWW_ROOT'].'index.php?t=pmuserloc&amp;'._rsid.'&amp;js_redr=buddy_add.add_login&amp;overwrite=1\&#39;, \&#39;user_list\&#39;, 325,250);">Поиска</a> для нахождения нужного пользователя.' : ''); ?><p>

...
function make_perms_query(&$fields, &$join, $fid='')
{
	...
	ses_update_status($usr->sid, 'Поиск пользователей');
	...
	if ($usr->users_opt & 524288 || $is_a) {	// is_mod or admin.
		if ($is_a) {
			...
			if ($report_count = q_singleval('SELECT count(*) FROM fud26_msg_report')) {
				$reported_msgs = '| <a href="index.php?t=reported&amp;'._rsid.'">Извещения о сообщениях</a> <span class="GenTextRed">('.$report_count.')</span>';
			}
			...
			if ($FUD_OPT_2 & 1024 && ($accounts_pending_approval = q_singleval('SELECT count(*) FROM fud26_users WHERE users_opt>=2097152 AND '. q_bitand('users_opt', 2097152) .' > 0 AND id > 0'))) {
				$accounts_pending_approval = '| <a href="adm/admaccapr.php?S='.s.'&amp;SQ='.$GLOBALS['sq'].'">Ожидающие утверждения пользователи</a> <span class="GenTextRed">('.$accounts_pending_approval.')</span>';
			} else {
...
<div class="GenText ac">[<a href="index.php?t=actions&rand=<?php echo get_random_value(); ?>&<?php echo _rsid; ?>">Обновить список</a>]</div>
<p>
<table cellspacing="1" cellpadding="2" class="ContentTable">
<tr><th>Пользователь</th><th>Действие</th><th>Время</th><th>Последнее сообщение</th></tr>

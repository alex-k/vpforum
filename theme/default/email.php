...
* $Id: email.php.t 4994 2010-09-02 17:33:29Z naudefj $
*
...
}class fud_smtp
{
	...
	if (!($FUD_OPT_2 & 1073741824)) {
		error_dialog('ОШИБКА: Невозможно отправить электронное сообщение', 'Система отправки e-mail была выключена администратором.');
	}
...
function mail_check()
{
	if (!strlen(trim($_POST['tx_body']))) {
		set_err('tx_body', 'Текст сообщения обязателен');
	}

	if (!strlen(trim($_POST['tx_subject']))) {
		set_err('tx_subject', 'Тема обязательна');
	}

	if (!strlen(trim($_POST['tx_name']))) {
		set_err('tx_name', 'Имя обязательно');
	} else if (!q_singleval('SELECT id FROM fud26_users WHERE alias='. _esc(htmlspecialchars($_POST['tx_name'])))) {
		set_err('tx_name', 'Недопустимый пользователь');
	}
	...
	if (isset($_POST['btn_submit'])) {
		if (!mail_check()) {
			if (!($email = q_singleval('SELECT email FROM fud26_users WHERE alias='. _esc(htmlspecialchars($_POST['tx_name'])) .' AND (users_opt & 16) > 0'))) {
				error_dialog('ОШИБКА: Невозможно отправить электронное сообщение', 'Извините, система не может отправить электронную почту '.htmlspecialchars($_POST['tx_name']).', так как не может установить адрес получателя');
			}
...
<form method="post" id="frme" action="<?php echo $GLOBALS['WWW_ROOT']; ?>index.php?t=email">
<div class="ctb"><table cellspacing="1" cellpadding="2" class="MiniTable">
<tr><th colspan="2">Электронная почта</th></tr>
<tr class="RowStyleB"><td class="GenText nw">Кому: <?php echo get_err('tx_name'); ?></td><td class="RowStyleA wa"><input type="text" name="tx_name" value="<?php echo htmlspecialchars($tx_name); ?>" /></td></tr>
<tr class="RowStyleB"><td class="GenText nw">Тема: <?php echo get_err('tx_subject'); ?></td><td class="RowStyleA wa"><input spellcheck="true" type="text" tabindex="1" name="tx_subject" value="<?php echo (isset($_POST['tx_subject']) ? htmlspecialchars($_POST['tx_subject']).'' : ''); ?>" /></td></tr>
<tr class="RowStyleB"><td class="GenText nw vt">Текст: <?php echo get_err('tx_body'); ?></td><td class="RowStyleA wa"><textarea tabindex="2" name="tx_body" cols="40" rows="20"><?php echo (isset($_POST['tx_body']) ? htmlspecialchars($_POST['tx_body']).'' : ''); ?></textarea></td></tr>
<tr class="RowStyleC"><td class="ar" colspan="2"><input type="submit" class="button" name="btn_submit" tabindex="3" value="Электронная почта" /></td></tr>

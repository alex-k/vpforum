	...
	while ($obj = db_rowobj($c)) {
		...
		if ($FUD_OPT_2 & 32 && (!($obj->users_opt & 32768) || $is_a)) {
			...
			if (($obj->time_sec + $LOGEDIN_TIMEOUT * 60) > __request_timestamp__) {
				$online_indicator = '<img src="theme/default/images/online'.img_ext.'" alt="онлайн" title="онлайн" />&nbsp;';
			} else {
				$online_indicator = '<img src="theme/default/images/offline'.img_ext.'" alt="оффлайн" title="оффлайн" />&nbsp;';
			}
		} else {
			$online_indicator = '';
		}
...
<td>'.($obj->read_stamp ? '<img src="theme/default/images/pmsg_read'.img_ext.'" alt="Прочитанное личное сообщение" title="Прочитанное личное сообщение" />' : '<img src="theme/default/images/pmsg_unread'.img_ext.'" alt="Непрочитанное личное сообщение" title="Непрочитанное личное сообщение" />' ) .'</td><td width="100%" class="GenText">'.$msg_type.'<a href="'.$goto.'">'.$obj->subject.'</a>'.(($obj->pmsg_opt & 4 && $obj->pmsg_opt & 16 && $obj->duser_id == _uid && $obj->ouser_id != _uid) ? '<span class="SmallText">&nbsp;&nbsp;[<a href="index.php?t=pmsg_view&amp;'._rsid.'&amp;dr=1&amp;id='.$obj->id.'" title="Не отправлять извещение автору о прочтении вами данного сообщения">запретить извещение</a>]</span>' : '' ) .'</td>

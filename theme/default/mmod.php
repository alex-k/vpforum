...
* $Id: mmod.php.t 4994 2010-09-02 17:33:29Z naudefj $
*
...
if (__fud_real_user__ && $FUD_OPT_1 & 1024) {	// PM_ENABLED
	...
	if ($del) {
		...
		if (empty($_POST['confirm'])) {
			if ($data[2] != $data[4]) {
				$delete_msg = 'Вы собираетесь <span class="ErrorText">удалить</span> сообщение, озаглавленное: <b>'.$data[3].'</b>';
			} else {
				$delete_msg = 'Вы собираетесь <span class="ErrorText">удалить всю тему</span>, озаглавленную: <b>'.$data[3].'</b>';
			}
...
<tr><th>Подтверждение удаления темы/сообщения</th></tr>
...
<b>Дополнительно</b><br />Укажите причину удаления сообщения, которое будет отправлено автору через личную почту.<br />
<textarea rows="10" cols="80" name="del_reason">

модератор не счел нужным указать причину удаления

</textarea><br />
<label><input type="checkbox" name="del_inc_body" /> Включить содержимое удаленного сообщения в письмо, отправляемое автору.</label>
<br /><br />
</div>
<br />
<font size="3"><b>Вы действительно хотите продолжить?</b><br /></font><br />
<input type="submit" class="button" name="YES" value="Да" />&nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" class="button" name="NO" value="Нет" />
</form>
</td></tr>
</table></div>
...
'.$body;
				...
				send_status_update($data[12], $data[13], $data[14], 'Ваше сообщение было удалено администрацией.', $body);
			}

...
function reverse_fmt($data)
{
	...
	$TITLE_EXTRA = ': Редактор голосований';



	$pl_expiry_date_data = tmpl_draw_select_opt("0\n3600\n21600\n43200\n86400\n259200\n604800\n2635200\n31536000", "Не ограничено\n1 час\n6 часов\n12 часов\n1 день\n3 дня\n1 неделя\n1 месяц\n1 год", $pl_expiry_date);
	$pl_max_votes_data = tmpl_draw_select_opt("0\n10\n50\n100\n200\n500\n1000\n10000\n100000", "Не ограничено\n10\n50\n100\n200\n500\n1000\n10000\n100000", $pl_max_votes);

	if ($frm->group_cache_opt & 16384) {
		$checked = isset($_POST['pl_smiley_disabled']) ? ' checked' : '';
		$pl_smiley_disabled_chk = '<tr><td align="right" valign="top" colspan="2" class="GenText"><label><input type="checkbox" name="pl_smiley_disabled" value="Y" checked="'.$checked.'" />Запретить смайлики</label></td></tr>';
	} else {
	...
	if ($pl_id) {
		...
		foreach ($poll_opts as $k => $v) {
			$poll_option_entry_data .= '<tr><td class="GenText">'.$v.'</td><td style="white-space: nowrap">[<a href="index.php?t=poll&amp;frm_id='.$frm_id.'&amp;'._rsid.'&amp;pl_id='.$pl_id.'&amp;pl_optedit='.$k.'">Правка</a>] [<a href="index.php?t=poll&amp;frm_id='.$frm_id.'&amp;pl_id='.$pl_id.'&amp;del_id='.$k.'&amp;'._rsid.'">Удалить</a>]</td></tr>';
		}

		$poll_editor = '<table width="99%" cellspacing="2" cellpadding="0" class="dashed">
<tr><td class="GenText">Добавить вариант:</td><td class="ar"><input type="text" spellcheck="true" name="pl_option" value="'.htmlspecialchars($pl_option).'" />
'.$pl_smiley_disabled_chk.'
<tr><td colspan="2" class="ar">'.(!isset($_GET['pl_optedit']) ? '<input type="submit" class="button" name="pl_add" onclick="return check_submission();" value="Добавить вариант" />' : '<input type="hidden" name="pl_option_id" value="'.$pl_option_id.'" />
<input type="submit" class="button" name="pl_upd" onclick="return check_submission();" value="Изменить вариант" />' ) .'</td></tr>
...
<table cellspacing="2" width="99%" cellpadding="0" class="dashed">
	<tr>
		<td class="GenText">Название голосования:</td>
		<td><input tabindex="1" type="text" spellcheck="true" name="pl_name" value="<?php echo htmlspecialchars($pl_name); ?>" /></td>
	</tr>
	<tr>
		<td class="GenText">Время действия:</td>
		<td><select tabindex="2" name="pl_expiry_date"><?php echo $pl_expiry_date_data; ?></select></td>
	</tr>
	<tr>
		<td class="GenText">Максимум голосов:</td>
		<td><select tabindex="3" name="pl_max_votes"><?php echo $pl_max_votes_data; ?></select></td>
	</tr>

	<tr><td colspan="2" class="ar"><?php echo (!$pl_id ? '<input tabindex="5" type="submit" class="button" name="pl_submit" value="Создать голосование" />' : '<input tabindex="5" type="submit" class="button" name="pl_submit" value="Изменить голосование" />'); ?></td></tr>
</table>
<br />
<?php echo $poll_editor; ?>
<br />
<div class="ar">
<?php echo (!$pl_id ? '<input type="button" tabindex="4" class="button" onclick="window.opener.document.forms[\'post_form\'].pl_id.value=\'0\'; window.opener.document.forms[\'post_form\'].submit(); window.close();" value="Создать" />' : '<input type="button" tabindex="4" class="button" onclick="window.opener.document.forms[\'post_form\'].pl_id.value='.$pl_id.'; window.opener.document.forms[\'post_form\'].submit(); window.close();" value="Изменить" />'); ?>
</div>
</form>
...
function check_submission()
{
	...
	if (!a.length) {
		if (document.forms['poll_submit'].pl_add && document.forms['poll_submit'].pl_add.value.length) {
			alert("Вы не можете добавить пустой вариант.");
			return false;
		} else if (document.forms['poll_submit'].pl_upd && document.forms['poll_submit'].pl_upd.value.length) {
			return confirm("Если вы хотите удалить вариант, нажмите OK");
		}
	}
	return true;
}

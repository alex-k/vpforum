...
<span class="CatLockedName"><a href="index.php?t=index&amp;cat='.$k.'&amp;'._rsid.'" class="CatLink">'.$i[1].'</a></span> '.$i[2].'</td><td class="CatDesc">
	...
	<td class="RowStyleA" colspan="6">'.$r[10].($r[11] ? '<br />'.$r[11] : '').'</td>
		...
		if (!_uid) { /* Anon user. */
			$forum_read_indicator = '<img title="Только зарегистрированные участники форума могут отслеживать и читать &amp; новые сообщений" src="theme/default/images/existing_content'.img_ext.'" alt="Только зарегистрированные участники форума могут отслеживать и читать &amp; новые сообщений" />';
		} else if ($r[15] < $r[2] && $usr->last_read < $r[2]) {
			$forum_read_indicator = '<img title="Новые сообщения" src="theme/default/images/new_content'.img_ext.'" alt="Новые сообщения" />';
		} else {
			$forum_read_indicator = '<img title="Нет новых сообщений" src="theme/default/images/existing_content'.img_ext.'" alt="Нет новых сообщений" />';
		}
	...
	<td class="RowStyleB ac wo">'.(empty($r[12]) ? $forum_read_indicator.'' : '<img title="Перенаправление" src="theme/default/images/moved'.img_ext.'" alt="" />' )  .'</td>
	...
	<td class="RowStyleA ac nw">'.(empty($r[12]) ? ($r[8] ? '<span class="DateText">'.strftime("%a, %d %B %Y", $r[2]).'</span><br />От: '.($r[3] ? '<a href="index.php?t=usrinfo&amp;id='.$r[3].'&amp;'._rsid.'">'.$r[4].'</a>' : $GLOBALS['ANON_NICK'].'' ) .' <a href="index.php?t='.d_thread_view.'&amp;goto='.$r[8].'&amp;'._rsid.'#msg_'.$r[8].'"><img title="'.$r[0].'" src="theme/default/images/goto.gif" alt="'.$r[0].'" /></a>' : 'н/д' ) : '--' )  .'</td>
</tr>';
	}
	unset($c);
...
'.($FUD_OPT_2 & 1048576 ? '[ <a href="feed.php?mode=m&amp;l=1&amp;basic=1"><img src="theme/default/images/rss.gif" title="Сформировать XML" alt="Сформировать XML"/></a> ]' : '' )  .'
</div>' : ''); ?>
...
<img src="theme/default/images/new_content<?php echo img_ext; ?>" alt="Есть новые сообщения с момента последнего посещения" /> Есть новые сообщения с момента последнего посещения&nbsp;&nbsp;
<img src="theme/default/images/existing_content<?php echo img_ext; ?>" alt="Нет новых сообщения с момента последнего посещения" /> Нет новых сообщения с момента последнего посещения&nbsp;&nbsp;
<img src="theme/default/images/moved<?php echo img_ext; ?>" alt="Перенаправление" /> Перенаправление
</fieldset>
...
/* <![CDATA[ */
min_max_cats("theme/default/images", "<?php echo img_ext; ?>", "Свернуть категорию", "Развернуть категорию", "<?php echo $usr->sq; ?>", "<?php echo s; ?>");
/* ]]> */
</script>
</td>

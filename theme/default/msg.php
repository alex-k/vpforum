...
' : '' )  ;
	...
	if (isset($_GET['unread']) && $th && _uid) {
		$_GET['goto'] = q_singleval('SELECT m.id from fud26_msg m LEFT JOIN fud26_read r ON r.thread_id=m.thread_id AND r.user_id='. _uid .' WHERE m.thread_id='. $th .' AND m.apr=1 AND m.post_stamp>CASE WHEN (r.last_view IS NOT NULL OR r.last_view>'. $usr->last_read .') THEN r.last_view ELSE '. $usr->last_read .' END order by id limit 1');
		if (!$_GET['goto']) {
...
<span id="threadRating"><?php echo ($FUD_OPT_2 & 4096 && $frm->rating ? '&nbsp;('.($MOD ? '<a href="javascript://" onclick="window_open(\''.$GLOBALS['WWW_ROOT'].'index.php?t=ratingtrack&amp;'._rsid.'&amp;th='.$frm->id.'\', \'th_rating_track\', 300, 400);">' : '' )  .'<img src="theme/default/images/'.$frm->rating.'stars.gif" title="'.$frm->rating.' из '.convertPlural($frm->n_rating, array(''.$frm->n_rating.' голоса',''.$frm->n_rating.' голосов',''.$frm->n_rating.' голосов')).'" alt=""/>'.($MOD ? '</a>' : '' )  .') '.convertPlural($frm->n_rating, array(''.$frm->n_rating.' голос',''.$frm->n_rating.' голоса',''.$frm->n_rating.' голосов')).'' : ''); ?></span>
...
<?php echo $first_unread_message_link.$subscribe_status.$bookmark_status; ?>
 ...
 <td class="vb ar" rowspan=2>
 <?php echo ($FUD_OPT_3 & 2 ? '' : '<a href="index.php?t=tree&amp;th='.$th.'&amp;'._rsid.'"><img title="древовидный вид" alt="древовидный вид" src="theme/default/images/tree_view.gif" /></a>&nbsp;'); ?><a href="index.php?t=post&amp;frm_id=<?php echo $frm->forum_id; ?>&amp;<?php echo _rsid; ?>"><img alt="Создать новую тему" src="theme/default/images/new_thread.gif" /></a><?php echo ((!($frm->thread_opt & 1) || $perms & 4096) ? '&nbsp;<a href="index.php?t=post&amp;th_id='.$th.'&amp;reply_to='.$frm->root_msg_id.'&amp;'._rsid.'&amp;start='.$_GET['start'].'"><img src="theme/default/images/post_reply.gif" alt="Отправить ответ" /></a>' : ''); ?>
 </td>


</tr>
<tr>
<td class="GenText al" colspan="2">
...
<tr><td class="MsgR1 fb '.(!$quick_reply_collapsed ? 'MsgSubText' : '' )  .'">'.($quick_reply_collapsed ? '<img src="theme/default/images/max'.img_ext.'" alt="+" title="Максимайз" class="collapsable" />' : '' )  .' быстрый ответ</td></tr>
<tr><td class="MsgR3" '.($quick_reply_collapsed ? 'style="display:none;"' : '' )  .'>
	<table><tr class="RowStyleA"><td class="GenText nw">Формат:</td><td>
...
<td class="vt">
'.$page_pager.'&nbsp;</td>
<td class="GenText ar vb nw">'.($FUD_OPT_3 & 2 ? '' : '<a href="index.php?t=tree&amp;th='.$th.'&amp;'._rsid.'"><img title="древовидный вид" alt="древовидный вид" src="theme/default/images/tree_view.gif" /></a>&nbsp;'); ?><a href="index.php?t=post&amp;frm_id=<?php echo $frm->forum_id; ?>&amp;<?php echo _rsid; ?>"><img alt="Создать новую тему" src="theme/default/images/new_thread.gif" /></a><?php echo (!$quick_reply_enabled && (!($frm->thread_opt & 1) || $perms & 4096) ? '&nbsp;<a href="index.php?t=post&amp;th_id='.$th.'&amp;reply_to='.$frm->root_msg_id.'&amp;'._rsid.'&amp;start='.$_GET['start'].'"><img src="theme/default/images/post_reply.gif" alt="Отправить ответ" /></a>' : ''); ?></td>
</tr>
...
<div class="ar SmallText"><?php echo ($FUD_OPT_2 & 1048576 ? '[ <a href="index.php?t=help_index&amp;section=boardusage#syndicate">Сформировать XML</a> ] [ <a href="feed.php?mode=m&amp;th='.$th.'&amp;basic=1"><img src="theme/default/images/rss.gif" title="Сформировать XML" alt="RSS" /></a> ]' : ''); ?> <?php echo ($FUD_OPT_2 & 2097152 ? '[ <a href="'.$GLOBALS['WWW_ROOT'].'pdf.php?th='.$th.'&amp;'._rsid.'"><img src="theme/default/images/pdf.gif" title="Сформировать PDF для печати" alt="PDF" /></a> ]' : ''); ?></div>
...
/* <![CDATA[ */
min_max_posts('theme/default/images', '<?php echo img_ext; ?>', 'Минимайз', 'Максимайз');

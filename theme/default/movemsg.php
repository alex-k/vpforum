...
* $Id: movemsg.php.t 4994 2010-09-02 17:33:29Z naudefj $
*
...
function db_assoc($q)
{
	...
	if (!($sth_info = db_arr_assoc('SELECT forum_id, root_msg_id, replies, last_post_id, last_post_date FROM fud26_thread WHERE id='. $th))) {
		invl_inp_err();
	}	
	...
	if (!empty($_POST['dest_th']) && !empty($_POST['msg_ids'])) {
		...
		if ($mids && $dth > 0) {
			$mids = db_all('SELECT id FROM fud26_msg WHERE id IN('. implode(',', $mids) .') AND thread_id='. $th);
		}
		if ($mids && $dth > 0) {
			if (!($th_info = db_arr_assoc('SELECT forum_id, root_msg_id, replies, last_post_id, last_post_date FROM fud26_thread WHERE id='. $dth))) {
			...
			$c_mids = count($mids);
			// Move thread.
			q('UPDATE fud26_msg SET thread_id='. $dth .' WHERE id IN('. $mstr .') AND thread_id='. $th);
			// Fix up reply_to.
			q('UPDATE fud26_msg SET reply_to='. $th_info['root_msg_id'] .' WHERE id IN('. $mstr .') AND reply_to NOT IN('. $mstr .')');

			// Determine if we need to update last_post_* in destination thread.
			$minfo = db_saq('SELECT id, post_stamp FROM fud26_msg WHERE id IN('. $mstr .') ORDER BY post_stamp DESC LIMIT 1');
			...
			q('UPDATE fud26_thread SET replies=replies+'. $c_mids . $pfx .' WHERE id='. $dth);
			if (q_singleval('SELECT last_post_id FROM fud26_forum WHERE id='. $dth) == $th_info['last_post_id']) {
				$pfx = ', last_post_id='.$minfo[0];
			} else {
				$pfx = '';
			}
			q('UPDATE fud26_forum SET post_count=post_count+'. $c_mids . $pfx .' WHERE id='. $th_info['forum_id']);

			// Update source thread.
			if ($sth_info['replies']+1 == $c_mids) { // Complete thread move.
				$del = new fud_msg_edit();
				$del->delete(1, q_singleval('SELECT root_msg_id FROM fud26_thread WHERE id='. $th), 1);
			} else {
				if (in_array($sth_info['last_post_id'], $mids)) {
					$sinfo = db_saq('SELECT id, post_stamp FROM fud26_msg WHERE thread_id='. $th .' ORDER BY post_stamp DESC LIMIT 1');
					q('UPDATE fud26_thread SET replies=replies-'. $c_mids .', last_post_date='. $sinfo[1] .', last_post_id='. $sinfo[0] .' WHERE id='. $th);
					rebuild_forum_view_ttl($sth_info['forum_id']);
					$lp = q_singleval('SELECT t.last_post_id FROM fud26_tv_'. $sth_info['forum_id'] .' v 
								INNER JOIN fud26_thread t ON t.id=v.thread_id
								WHERE seq=1');
					$pfx = ', last_post_id='. (int)$lp;
				} else {
					q('UPDATE fud26_thread SET replies=replies-'. $c_mids .' WHERE id='. $th);
					$pfx = '';
				}
				q('UPDATE fud26_forum SET post_count=post_count-'. $c_mids . $pfx .' WHERE id='. $th_info['forum_id']);
			}
...
<tr class="RowStyleA">
	<td class="al fb">Укажите ID темы для переноса в нее сообщений (может быть в других форумах)</td>
	<td class="al"><input type="text" value="" name="dest_th" /></td>
</tr>
<tr class="RowStyleC">
	<td colspan="2" class="ac"><input type="submit" class="button" name="btn_selected" value="Перенести выбранные сообщения" /></td>
</tr>
</table>
<br />
<table cellspacing="1" cellpadding="2" class="ContentTable">

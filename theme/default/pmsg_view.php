...
function tmpl_drawpmsg($obj, $usr, $mini)
{
	...
	if (!$mini) {
		...
		if (($o2 & 32 && !($a & 32768)) || $b & 1048576) {
			$obj->login = $obj->alias;
			$online_indicator = (($obj->last_visit + $GLOBALS['LOGEDIN_TIMEOUT'] * 60) > __request_timestamp__) ? '<img src="theme/default/images/online'.img_ext.'" alt="онлайн" title="онлайн" />' : '<img src="theme/default/images/offline'.img_ext.'" alt="оффлайн" title="оффлайн" />';
		} else {
			$online_indicator = '';
		}
		...
		if ($obj->ouser_id != _uid) {
			...
			$private_msg_link = '<a href="index.php?t=ppost&amp;toi='.$obj->user_id.'&amp;'._rsid.'"><img title="ЛС" src="theme/default/images/msg_pm.gif" alt="" /></a>';
		} else {
...
<td class="nw ar"><a href="index.php?t=pmsg&amp;'._rsid.'&amp;btn_delete=1&amp;sel='.$obj->id.'&amp;SQ='.$GLOBALS['sq'].'"><img src="theme/default/images/msg_delete.gif" alt="delete" /></a>&nbsp;'.($obj->fldr == 4 ? '<a href="index.php?t=ppost&amp;msg_id='.$obj->id.'&amp;'._rsid.'"><img src="theme/default/images/msg_edit.gif" alt="" /></a>&nbsp;&nbsp;&nbsp;&nbsp;' : '' )  .($obj->fldr == 1 ? '<a href="index.php?t=ppost&amp;reply='.$obj->id.'&amp;'._rsid.'"><img src="theme/default/images/msg_reply.gif" alt="reply" /></a>&nbsp;<a href="index.php?t=ppost&amp;quote='.$obj->id.'&amp;'._rsid.'"><img src="theme/default/images/msg_quote.gif" alt="quote" /></a>&nbsp;' : '' )  .'<a href="index.php?t=ppost&amp;forward='.$obj->id.'&amp;'._rsid.'"><img src="theme/default/images/msg_forward.gif" alt="forward" /></a></td>
</tr></table></td></tr>';
	} else {
...
<tr class="MsgR2">
'.$avatar.'
<td class="msgud">'.$online_indicator.(!$mini ? '<a href="index.php?t=usrinfo&amp;id='.$obj->user_id.'&amp;'._rsid.'">'.$obj->alias.'</a>' : $obj->alias )  .(!$mini ? '<br /><b>Сообщений:</b> '.$obj->posted_msg_count.'<br /><b>Зарегистрирован:</b> '.strftime("%B %Y", $obj->join_date).' '.$location : '' )  .'</td>
<td class="msgud">'.$dmsg_tags.'</td>
<td class="msgot">'.$buddy_link.$dmsg_im_row.(!$mini && $obj->host_name && $o1 & 268435456 ? '<b>От:</b> '.$obj->host_name.'<br />' : '' )  .'</td>
</tr></table></td>
</tr>
<tr><td class="MsgR3" colspan="2">
'.$msg_body.'
'.$file_attachments.'
...
<tr>'.$tabs.'</tr>
	...
	if (($nid = q_singleval('SELECT p.id FROM fud26_pmsg p INNER JOIN fud26_users u ON u.id=p.ouser_id WHERE p.duser_id='. _uid .' AND p.fldr='. $m->fldr .' AND post_stamp>'. $m->post_stamp .' ORDER BY p.post_stamp ASC LIMIT 1'))) {
		$dpmsg_next_message = '<a href="index.php?t=pmsg_view&amp;'._rsid.'&amp;id='.$nid.'">Следующее сообщение <img src="theme/default/images/goto.gif" alt="" /></a>';
	} else {
		$dpmsg_next_message = '';
	}
	...
	if (($pid = q_singleval('SELECT p.id FROM fud26_pmsg p INNER JOIN fud26_users u ON u.id=p.ouser_id WHERE p.duser_id='. _uid .' AND p.fldr='. $m->fldr .' AND p.post_stamp<'. $m->post_stamp .' ORDER BY p.post_stamp DESC LIMIT 1'))) {
		$dpmsg_prev_message = '<a href="index.php?t=pmsg_view&amp;'._rsid.'&amp;id='.$pid.'"><img src="theme/default/images/goback.gif" alt="" /> Предыдущее сообщение</a>';
	} else {
		$dpmsg_prev_message = '';
	}

							...
							if ($TREE_THREADS_MAX_DEPTH > $lev) {
								...
								if (_uid) {
									if ($usr->last_read < $cur->post_stamp && $cur->post_stamp>$cur->last_view) {
										$thread_read_status = $cur->thread_opt & 1 ? '<img src="theme/vp1/images/unreadlocked'.img_ext.'" title="Закрытая тема с непрочитанными сообщениями" alt="" />'	: '<img src="theme/vp1/images/unread'.img_ext.'" title="Эта тема содержит не прочитанные вами сообщения" alt="" />';
									} else {
										$thread_read_status = $cur->thread_opt & 1 ? '<img src="theme/vp1/images/readlocked'.img_ext.'" title="Эта тема была закрыта" alt="" />' : '<img src="theme/vp1/images/read'.img_ext.'" title="В этой теме нет непрочитанных сообщений" alt="" />';
									}
								} else {
									$thread_read_status = $cur->thread_opt & 1 ? '<img src="theme/vp1/images/readlocked'.img_ext.'" title="Эта тема была закрыта" alt="" />' : '<img src="theme/vp1/images/read'.img_ext.'" title="Отслеживание прочитанных и непрочитанных сообщений доступно только зарегистрированным участникам" alt="" />';
								}
...
<td title="'.$cur->tdescr.'" class="tt" style="padding-left: '.(20 * ($lev - 1)).'px">'.($cur->poll_id ? 'Голосование:&nbsp;' : '' ) .($cur->attach_cnt ? '<img src="theme/vp1/images/attachment.gif" alt="" />' : '' ) .'<a href="index.php?t='.d_thread_view.'&amp;goto='.$cur->id.'&amp;'._rsid.'#msg_'.$cur->id.'" class="big">'.$cur->subject.'</a>
...
<td class="GenText nw vb ar"><a href="index.php?t=thread&amp;frm_id=<?php echo $frm->id; ?>&amp;<?php echo _rsid; ?>"><img alt="Вернуться к плоскому виду по умолчанию" title="Вернуться к плоскому виду по умолчанию" src="theme/vp1/images/flat_view.gif" /></a>&nbsp;<a href="index.php?t=post&amp;frm_id=<?php echo $frm->id; ?>&amp;<?php echo _rsid; ?>"><img src="theme/vp1/images/new_thread.gif" alt="Создать новую тему" /></a></td>
</tr></table>
...
[ <a href="feed.php?mode=m&amp;l=1&amp;basic=1&amp;frm='.$frm->id.'&amp;n=10"><img src="theme/vp1/images/rss.gif" title="Сформировать XML" alt="RSS" /></a> ]' : '' ) .(($FUD_OPT_2 & 270532608) == 270532608 ? '&nbsp;[ <a href="'.$GLOBALS['WWW_ROOT'].'pdf.php?frm='.$frm->id.'&amp;page='.$cur_frm_page.'&amp;'._rsid.'"><img src="theme/vp1/images/pdf.gif" title="Сформировать PDF для печати" alt="PDF" /></a> ]' : '' )  .'</div>' : '<div class="ar SmallText">'.(($FUD_OPT_2 & 270532608) == 270532608 ? '&nbsp;[ <a href="'.$GLOBALS['WWW_ROOT'].'pdf.php?frm='.$frm->id.'&amp;page='.$cur_frm_page.'&amp;'._rsid.'"><img src="theme/vp1/images/pdf.gif" title="Сформировать PDF для печати" alt="PDF" /></a> ]' : '' ) .($FUD_OPT_2 & 1048576 ? '&nbsp;[ <a href="index.php?t=help_index&amp;section=boardusage#syndicate">Сформировать XML</a> ]

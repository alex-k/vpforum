...
<tr>'.$tabs.'</tr>
	...
	if (($r = db_rowarr($c))) {
		...
		do {
			if ((!($r[5] & 32768) && $FUD_OPT_2 & 32) || $is_a) {
				$online_status = (($r[8] + $LOGEDIN_TIMEOUT * 60) > __request_timestamp__) ? '<img src="theme/vp1/images/online'.img_ext.'" title="'.$r[2].' сейчас в онлайне" alt="'.$r[2].' сейчас в онлайне" />' : '<img src="theme/vp1/images/offline'.img_ext.'" title="'.$r[2].' сейчас не в онлайне" alt="'.$r[2].' сейчас не в онлайне" />';
			} else {
				$online_status = '';
			}
			...
			if ($r[4] && substr($r[4], 0, 4) == $md) {
				...
				$bday_indicator = '<img src="blank.gif" alt="" width="10" height="1" /><img src="theme/vp1/images/bday.gif" alt="" />Сегодня '.$r[2].' исполняется '.$age;
			} else {
				$bday_indicator = '';
			}
	...
	<td class="GenText nw"><a href="index.php?t=usrinfo&amp;id='.$r[1].'&amp;'._rsid.'"><img src="theme/vp1/images/msg_about.gif" alt="" /></a>&nbsp;<a href="index.php?t=showposts&amp;'._rsid.'&amp;id='.$r[1].'"><img src="theme/vp1/images/show_posts.gif" alt="" /></a> '.($r[7] ? '<a href="'.$r[7].'"><img src="theme/vp1/images/homepage.gif" alt="" /></a>' : '' ) .'</td>
</tr>';
		} while (($r = db_rowarr($c)));

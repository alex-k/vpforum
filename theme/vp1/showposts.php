...
<tr>'.$tabs.'</tr>
	...
	$TITLE_EXTRA = ': Показать сообщения от: '.$u_alias;

	ses_update_status($usr->sid, 'Просмотр всех сообщений от <a href="index.php?t=usrinfo&amp;id='.$uid.'">'.$u_alias.'</a>');
	...
	if ($is_a || $fids) {
		...
		while ($r = db_rowarr($c)) {
			$post_entry .= '<tr class="'.alt_var('search_alt','RowStyleA','RowStyleB').'"><td class="GenText"><a href="index.php?t='.d_thread_view.'&amp;goto='.$r[3].'&amp;'._rsid.'#msg_'.$r[3].'">'.$r[2].'</a></td><td class="GenText" style="white-space: nowrap"><a href="index.php?t='.t_thread_view.'&amp;frm_id='.$r[1].'&amp;'._rsid.'">'.$r[0].'</a></td><td class="DateText nw">'.strftime("%a, %d %B %Y %H:%M", $r[4]).'</td></tr>';
		}
		unset($c);
...
<tr class="RowStyleA"><td class="GenText wa">Учётная запись: <a href="index.php?t=usrinfo&amp;id=<?php echo $uid; ?>&amp;<?php echo _rsid; ?>"><?php echo $u_alias; ?></a></td>
<td class="GenText nw">Дата регистрации: <?php echo strftime("%a, %d %B %Y", $u_reg_date); ?></td><td class="GenText nw">Количество сообщений <?php echo $u_pcount; ?></td></tr>
<tr><th class="wa">Тема</th><th class="nw">Форум:</th><th class="nw"><a class="thLnk" href="index.php?t=showposts&amp;so=<?php echo $SORT_ORDER_R; ?>&amp;id=<?php echo $uid; ?>&amp;<?php echo _rsid; ?>" rel="nofollow">Дата отправки</a></th></tr>
<?php echo $post_entry; ?>
</table>
<?php echo $pager; ?>
